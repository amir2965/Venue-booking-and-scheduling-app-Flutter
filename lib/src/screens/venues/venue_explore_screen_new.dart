...
class _VenueExploreScreenState extends ConsumerState<VenueExploreScreen>
  ...
  void initState() {
    ...
    // Create animation for icon size
    _iconSizeAnimation = Tween<double>(begin: 1.0, end: 0.0).animate(
      CurvedAnimation(
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: CustomScrollView(
        ...
        slivers: [
          // App Bar (without title)
          SliverAppBar(
            expandedHeight:
                176, // Adjusted to bring sports filters closer to bottom edge
            floating: false,
            pinned: true,
            backgroundColor: const Color(0xFFF8F8F7), // Match main background
            surfaceTintColor:
                const Color(0xFFF8F8F7), // Prevent color change on scroll
            scrolledUnderElevation: 0, // Prevent elevation change on scroll
            elevation: 0,
            automaticallyImplyLeading: false, // Remove back button
            bottom: PreferredSize(
              preferredSize: const Size.fromHeight(
                  150), // Reduced to bring sports filters even closer to bottom edge
              child: Container(
                padding: const EdgeInsets.fromLTRB(0, 20, 0,
                    0), // Removed horizontal padding to allow sports filter to extend to edges
                child: Column(
                  children: [
                    // Search Bar
                    Container(
                      ...
                      margin: const EdgeInsets.symmetric(
                          horizontal: 24), // Added margin for search bar only
                      decoration: BoxDecoration(
                        color: const Color(0xFFF8F8F7), // Same as background
                        borderRadius:
                      ...
                      child: ClipRRect(
                        borderRadius: BorderRadius.circular(
                            28), // Match container's border radius
                        child: Stack(
                          children: [
                            TextField(
                    ...
                    // Sports Filter Row with shrinking animation
                    AnimatedBuilder(
          ...
          SliverToBoxAdapter(
            child: _buildHorizontalVenueList(
              allVenues.when(
                data: (venues) => topRatedVenues.take(5).toList(),
                loading: () => List.generate(
                  3,
                  (index) => Venue.skeleton(),
                ),
          ...
          SliverToBoxAdapter(
            child: _buildHorizontalVenueList(
              allVenues.when(
                data: (venues) => trendingVenues.take(5).toList(),
                loading: () => List.generate(
                  3,
                  (index) => Venue.skeleton(),
                ),
          ...
          if (_selectedSport.isNotEmpty) ...[
            ...
            SliverToBoxAdapter(
              child: _buildHorizontalVenueList(
                allVenues.when(
                  data: (venues) => filteredVenues.take(5).toList(),
                  loading: () => List.generate(
                    3,
                    (index) => Venue.skeleton(),
                  ),
